...
/**
 * Snapshot and restore MobilityCheck data before app updates.
 *
...
{
	private const LOCK_KEY = 'mobilitycheck-upgrade-backup';
	...
	{
		return $this->runExclusive('MobilityCheck upgrade backup', function () use ($reason, $rotate, $rotationExcludeIds): array {
	...
	private function doCreateSnapshot(string $reason, bool $rotate, array $rotationExcludeIds): array
	{
		...
		if (!$this->hasDataToBackup()) {
			throw new UpgradeBackupException('No MobilityCheck tables exist yet; nothing to back up.');
		}
		...
		try {
			...
			if ($backedUpTables === []) {
				throw new UpgradeBackupException('No MobilityCheck tables exist on this instance; nothing to back up.');
			}
		...
		$this->logger->info('MobilityCheck: created pre-update backup snapshot', [
	...
	public function listSnapshots(): array
	{
		...
		foreach ($this->listSnapshotIds() as $snapshotId) {
			try {
				...
				$this->logger->warning('MobilityCheck: skipping unreadable upgrade backup folder', [
					'app' => UpgradeBackupCatalog::APP_ID,
					'folder' => $snapshotId,
					'exception' => $e,
				]);
	...
	public function restoreSnapshot(string $snapshotId, bool $createSafetySnapshot = true): void
	{
		...
		$this->runExclusive('MobilityCheck upgrade restore', function () use ($snapshotId, $folder, $tableManifest, $createSafetySnapshot): void {
	...
	private function doRestoreSnapshot(
		...
		if ($this->db->getDatabaseProvider() === IDBConnection::PLATFORM_MYSQL) {
			...
			try {
				...
				$this->logger->warning('MobilityCheck: Oracle constraints could not be deferred for restore; relying on restore table order.', [
					'exception' => $e,
				]);
			}
		}
		...
		try {
			...
			$this->db->rollBack();
			throw new UpgradeBackupException('MobilityCheck restore failed; database changes were rolled back.', 0, $e);
		} finally {
			if ($fkChecksDisabled) {
		...
		try {
			...
			throw new UpgradeBackupException(
				'MobilityCheck database was restored but app config or app data folders failed. '
		...
		$this->logger->warning('MobilityCheck: restored data from upgrade backup snapshot', [
			'app' => UpgradeBackupCatalog::APP_ID,
	...
	private function getAppDataRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('MobilityCheck app data root is not a folder.');
		}
	...
	private function getBackupRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('MobilityCheck upgrade backup root is not a folder.');
		}
	...
	private function runExclusive(string $label, callable $callback)
	{
		try {
			...
			throw new UpgradeBackupException(
				'Another MobilityCheck backup or restore is already in progress.',
				0,
				$e,
			);
		}
	...
	private function purgeIncompleteSnapshotFolders(): void
	{
		foreach ($this->listSnapshotIds() as $snapshotId) {
			try {
				...
				$this->logger->warning('MobilityCheck: removing corrupt upgrade backup folder', [
					'app' => UpgradeBackupCatalog::APP_ID,
					'folder' => $snapshotId,
					'exception' => $e,
				]);
