...
/**
 * List, create, or restore MobilityCheck upgrade backups (CLI operator interface).
 *
 * @copyright Copyright (c) 2026, Nextcloud GmbH
 * @license AGPL-3.0-or-later
 */

namespace OCA\MobilityCheck\Command;

use OCA\MobilityCheck\Exception\UpgradeBackupException;
use OCA\MobilityCheck\Service\UpgradeBackupCatalog;
use OCA\MobilityCheck\Service\UpgradeBackupIntegrity;
...
{
	private const APP_LABEL = 'MobilityCheck';
	...
	{
		$this
			->setName('mobilitycheck:upgrade-backup')
			->setDescription('Manage MobilityCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
...
<info>create</info> — manual snapshot before risky changes:
  <comment>occ mobilitycheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ mobilitycheck:upgrade-backup restore --latest --force</comment>
  <comment>occ mobilitycheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current MobilityCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ mobilitycheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current MobilityCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ mobilitycheck:upgrade-backup restore --latest --force',
				'  occ mobilitycheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ mobilitycheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
